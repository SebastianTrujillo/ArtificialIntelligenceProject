...
class StarSearch(object):
    ...
    def calculateCost(self,nodeList,addCosts):
        path=[]
        while(nodeList!=None):
            path.append((nodeList[1],nodeList[2]))
            nodeList=nodeList[3]
        path.reverse()
        cost=len(path)+addCosts
        return(cost)
    ...
    def searchPath(self):
        ...
        while(queue.qsize()>0): #make sure there are nodes to check left
            ...
            for i in ([x,y-1],[x,y+1],[x-1,y],[x+1,y]): #new spots to try
                ...
                addCosts=0 #initializing the cost
                for index in range(len(self.costPosition)): #travel cost position
                    if([i[0],i[1]]==[self.costPosition[index][0],self.costPosition[index][1]]): #verify if the position of the cost
                        addCosts+=self.costValue #add the value of the cost
                queue.put((self.calculateCost(nodeList,addCosts)+self.calculateHeuristic(i[0],i[1]),i[0],i[1],nodeList)) #create the new spot, with node as the parent
