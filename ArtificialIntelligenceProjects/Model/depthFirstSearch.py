...
class DepthFirstSearch(object):
    ...
    def searchPath(self):
        stack=[]
        stack.append([self.startPosition[0],self.startPosition[1],None]) #create stack
        while(len(stack)>0): #make sure there are nodes to check left
            nodeList=stack.pop() #grab the first node
            x=nodeList[0] #get x
            y=nodeList[1] #get y
            ...
            for i in ([x,y-1],[x,y+1],[x-1,y],[x+1,y]): #new spots to try
                ...
                stack.append((i[0],i[1],nodeList)) #create the new spot, with node as the parent
